...
breed_dir_list = [name for name in os.listdir(images_dir)
...
if not list(processed_dir.glob('**/*.jpg')):
    for breed_dir in breed_dir_list:
        ...
        for breed_prefix in os.listdir(f'{annotation_dir}/{breed_dir}'):
            ...
            xmin = int(tree.getroot().findall('object')[0].find('bndbox').
                       find('xmin').text)
            xmax = int(tree.getroot().findall('object')[0].find('bndbox').
                       find('xmax').text)
            ymin = int(tree.getroot().findall('object')[0].find('bndbox').
                       find('ymin').text)
            ymax = int(tree.getroot().findall('object')[0].find('bndbox').
                       find('ymax').text)
